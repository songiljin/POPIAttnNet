...
class MHC_Read_data():
    ...
    def human_mhci_subclass(self,HLA_Subname,Sub_Number=None):
...
def mhc_data(mhc_type):
    
    read_data=MHC_Read_data('mhc.csv')
    if mhc_type == 'mhci_hla':
        human_mhc_positive,human_mhc_negative=    read_data.human_mhci_subclass('HLA-A')
    elif mhc_type == 'mhci':
        human_mhc_positive,human_mhc_negative=    read_data.human_mhci()    
    elif mhc_type == 'mhcii':
        human_mhc_positive,human_mhc_negative=    read_data.human_mhcii()    
        
    human_mhc_positive_seqs,human_mhc_negative_seqs=  read_data.get_sequences(
        human_mhc_positive,human_mhc_negative)   
    peptide_length = config.peptide_length
    processing_peptides=Processing_peptides()       
    all_peptide_seqs_mhc,mhc_label,positive_length=   processing_peptides.make_peptide_label(
        peptide_length,human_mhc_positive_seqs,human_mhc_negative_seqs)
    ...
    train_peptides,train_labels,positive_length,test_peptides,test_labels= make_train_test(
        all_peptide_seqs_mhc,mhc_label,positive_length,0.15)
