...
class POPIAttnNet(tf.keras.Model):
  ...
  def __init__(self,w1_units,MHCIP_units,batch_sz,fc_dim):
    ...
    self.bi_gru1 = tf.keras.layers.Bidirectional(tf.keras.layers.GRU(
                 ...
                 recurrent_dropout=0.5,recurrent_initializer='glorot_uniform',kernel_regularizer=tf.keras.regularizers.l1(0.005)),
        input_shape=(None,10,553))
    ...
    self.fc1 = tf.keras.layers.Dense(fc_dim,activation='relu',kernel_regularizer=tf.keras.regularizers.l1(0.005))
