import pandas as pd
import config
from HLA_sub import get_subtype_name_seq
...
class MHC_Read_data():
    ...
    def human_mhci_subclass(self,HLA_Subname,Sub_Number="02"):
        ...
        human_mhci_sub_positive=human_mhci_sub_positive[human_mhci_sub_positive
                            ['MHC_Restriction'].str.contains(HLA_Subname)]
        if Sub_Number:
            human_mhci_sub_positive=human_mhci_sub_positive[human_mhci_sub_positive
                            ['MHC_Restriction'].str.contains(Sub_Number)]
        ...
        human_mhci_sub_negative=human_mhci_sub_negative.fillna('nan', axis = 0)
        human_mhci_sub_negative=human_mhci_sub_negative.loc[lambda df:
                                 df.MHC_Restriction!='nan']
        #human_mhci_sub_negative=human_mhci_sub_negative.fillna('nan', axis = 0)
        #human_mhci_sub_negative=human_mhci_sub_negative[human_mhci_sub_negative
         #                   ['MHC_Restriction'].str.contains(HLA_Subname)]
        #human_mhci_sub_negative=human_mhci_sub_negative[human_mhci_sub_negative
         #                   ['MHC_Restriction'].str.contains(Sub_Number)]
    ...
    def human_mhci(self):
        ...
        human_mhci_positive=self.human_mhci_data.loc[lambda df:
                                        df.Immunogenicity=='Positive']
        human_mhci_positive=human_mhci_positive.fillna('nan', axis = 0)
        #human_mhci_positive=human_mhci_positive.loc[lambda df:
         #                        df.Immunogenicity_Evidence!='nan']

        human_mhci_negative=self.human_mhci_data.loc[lambda df:
                                        df.Immunogenicity=='Negative']
        human_mhci_negative=human_mhci_negative.fillna('nan', axis = 0)
        human_mhci_negative=human_mhci_negative.loc[lambda df:
                                 df.MHC_Restriction!='nan']
    ...
    def human_mhcii(self):
        ...
        human_mhcii_negative=self.human_mhcii_data.loc[lambda df:
                                        df.Immunogenicity=='Negative']
        human_mhcii_negative=human_mhcii_negative.fillna('nan', axis = 0)
        human_mhcii_negative=human_mhcii_negative.loc[lambda df:
                                 df.MHC_Restriction!='nan']
    ...
    def get_sequences(self,positive,negative):
         ...
         for pos_peptide,pos_hla_name in zip(positive['Peptide'],positive['MHC_Restriction']):
             positive_peptide_seqs.append(pos_hla_name+"xxx"+pos_peptide)
        
         for neg_peptide,neg_hla_name in zip(negative['Peptide'],negative['MHC_Restriction']):
             negative_peptide_seqs.append(neg_hla_name+"xxx"+neg_peptide)
...
file = "hla_prot.fasta"
subtype_name_seqs = get_subtype_name_seq(file)
class Processing_peptides():
    
    
    def length_filter(self,peptide_length,peptide_seqs,subtype_name_seqs):
        subtype_names = subtype_name_seqs.keys()
        filter_peptides=[]
        for peptide in peptide_seqs:
            hla_name = peptide.split("xxx")[0]
            if len(peptide.split("xxx")[1])==peptide_length:
                if hla_name.split("|")[0] in subtype_names:
                #peptide_r=peptide[::-1]
                    filter_peptides.append(peptide)
        return  filter_peptides
    
    def make_peptide_label(self,peptide_lengths,positive_seqs,negative_seqs):
        
        if peptide_lengths:
            positive_seqs=self.length_filter(peptide_lengths,positive_seqs,subtype_name_seqs)
            negative_seqs=self.length_filter(peptide_lengths,negative_seqs,subtype_name_seqs)
