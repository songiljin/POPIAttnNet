...
def peptide_into_property(peptides,filter_property_index,max_min=1):
    ...
    for peptide in peptides:
        i+=1
        print(i)
        hla_seq_name = peptide.split("xxx")[0]
        hla_seq_name = hla_seq_name.split("|")[0]
        file = "hla_prot.fasta"
        subtype_name_seq = get_subtype_name_seq(file)
        hla_seq = subtype_name_seq[hla_seq_name]
        peptide = peptide.split("xxx")[1]
        transfer_hla_seq = aaindex.amino_transfer_property(hla_seq,amino_property)
        transfer_hla_seq = np.array(transfer_hla_seq)
        transfer_hla_seq_mean = list(transfer_hla_seq.mean(axis=0))
        transfer_hla_seqs.append(transfer_hla_seq_mean)
    ...
    transfer_hla_seqs= np.array(transfer_hla_seqs)
    transfer_hla_seqs= np.expand_dims(transfer_hla_seqs,axis=1)
    transfer_peptides= np.hstack((transfer_peptides,transfer_hla_seqs))
